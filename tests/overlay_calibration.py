...
def load_left_images(left_img_dir):
    left_img_paths = sorted(left_img_dir.glob("*.jpg"))
...
def load_right_images(right_img_dir):
    right_img_paths = sorted(right_img_dir.glob("*.jpg"))
...
def setup_calibrator(ref_img_path, min_points=50):
    ...
    detector = pd.CharucoPlusChessboardPointDetector(
        ...
        size_of_charuco_squares=(5, 4),      # (outer square, inner square) in mm
...
if __name__ == "__main__":
    ...
    right_img_dir = base_dir / 'scikit-surgerycalibration' / 'tests'/ 'data'/ 'ChAruco_LR_frames_Steve_Axis_Tests'/ 'ExtractedFrames_R'
    
    results_folder = base_dir / 'data' / 'results' / 'calibrator_params'
    filename = results_folder / "stereo_calibration_params.json"
